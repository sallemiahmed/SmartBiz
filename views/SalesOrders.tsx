...
interface ViewEditModalProps {
  ...
  selectedAddItemId: string;
  customItem: { description: string; price: number; quantity: number };
  ...
  onSaveChanges: () => void;
  onEditDocChange: (doc: Invoice) => void;
  onEditItem: (index: number, field: keyof InvoiceItem, value: any) => void;
  onRemoveItem: (index: number) => void;
  onAddCatalogItem: () => void;
  onAddCustomItem: () => void;
  onSelectedAddItemIdChange: (id: string) => void;
  onCustomItemChange: (item: { description: string; price: number; quantity: number }) => void;
  onIsAddingCustomChange: (value: boolean) => void;
  getStatusColor: (status: string) => string;
}

const ViewEditModal = React.memo<ViewEditModalProps>(({
  isOpen,
  selectedDoc,
  editedDoc,
  isEditing,
  ...
  onAddCatalogItem,
  onAddCustomItem,
  onSelectedAddItemIdChange,
  onCustomItemChange,
  onIsAddingCustomChange,
  getStatusColor
}) => {
                  ...
                  <div>
                      ...
                      {isEditing ? (
                          <input
                              ...
                              onChange={(e) => onEditDocChange({...editedDoc, paymentTerms: e.target.value})}
                  ...
                  <div>
                      ...
                      {isEditing ? (
                          <select
                              ...
                              onChange={(e) => onEditDocChange({...editedDoc, paymentMethod: e.target.value})}
                              className="w-full bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded px-2 py-1 text-sm dark:text-white"
                          >
          ...
          <div className="border-t border-gray-200 dark:border-gray-700 pt-4 mt-4">
            ...
            {isEditing && (
                <div className="mb-4 space-y-3 bg-gray-50 dark:bg-gray-900/50 p-3 rounded-lg border border-gray-100 dark:border-gray-700">
                    ...
                    <div className="flex gap-2">
                        <div className="flex-1">
                            <SearchableSelect
                                ...
                                onChange={onSelectedAddItemIdChange}
                    ...
                    {!isAddingCustom ? (
                        <button
                            onClick={() => onIsAddingCustomChange(true)}
                        ...
                        <div className="space-y-2 animate-in fade-in slide-in-from-top-1">
                            <input
                                ...
                                onChange={(e) => onCustomItemChange({...customItem, description: e.target.value})}
                            ...
                            <div className="flex gap-2">
                                <input
                                    ...
                                    onChange={(e) => onCustomItemChange({...customItem, price: parseFloat(e.target.value) || 0})}
                                ...
                                <input
                                    ...
                                    onChange={(e) => onCustomItemChange({...customItem, quantity: parseInt(e.target.value) || 1})}
                                ...
                                <button
                                    onClick={() => onIsAddingCustomChange(false)}
              ...
              {isEditing ? (
                  editedDoc.items.map((item, idx) => (
                    <div key={idx} className="flex gap-2 items-center bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 p-2 rounded shadow-sm">
                        ...
                        <div className="flex items-center gap-1">
                            <input
                                id={`edit-item-quantity-${idx}`}
                                name={`itemQuantity-${idx}`}
                                type="number"
                                min="1"
                                value={item.quantity}
                                onChange={(e) => onEditItem(idx, 'quantity', parseInt(e.target.value) || 0)}
                            ...
                            <input
                                ...
                                step="0.01"
                                value={item.price}
                                onChange={(e) => onEditItem(idx, 'price', parseFloat(e.target.value) || 0)}
                                className="w-20 px-1 py-1 text-right text-sm border rounded dark:bg-gray-800 dark:border-gray-600 dark:text-white"
                            />
                        </div>
                        <button onClick={() => onRemoveItem(idx)} className="text-red-500 hover:text-red-700">
                  ...
                  selectedDoc.items.length > 0 ? (
                    selectedDoc.items.map((item, idx) => {
                      ...
                      const isFullyDeliveredItem = remaining <= 0;
                      return (
                        <div key={idx} className="flex justify-between items-center text-sm">
                          <div className="flex flex-col">
                            <span className="text-gray-600 dark:text-gray-400">{item.quantity}x {item.description}</span>
                            <span className={`text-xs ${isFullyDeliveredItem ? 'text-green-500' : 'text-orange-500'}`}>
                                {isFullyDeliveredItem ? 'Fully Delivered' : `Remaining: ${remaining}`}
                            </span>
                          </div>
...
ViewEditModal.displayName = 'ViewEditModal';
...
interface DeliveryModalProps {
  ...
  t: (key: string) => string;
  onClose: () => void;
  onDeliveryQtyChange: (id: string, value: string) => void;
  onSubmitDelivery: () => void;
}

const DeliveryModal = React.memo<DeliveryModalProps>(({
  isOpen,
  selectedDoc,
  deliveryQuantities,
  t,
                ...
                return (
                  <tr key={item.id} className={isFullyDelivered ? 'opacity-50' : ''}>
                    ...
                    <td className="px-3 py-3 text-right">
                      <input
                        id={`delivery-qty-${item.id}`}
                        name={`deliveryQty-${item.id}`}
                        type="number"
                        min="0"
                        max={remaining}
                        disabled={isFullyDelivered}
                        value={deliveryQuantities[item.id] || 0}
                        onChange={(e) => onDeliveryQtyChange(item.id, e.target.value)}
    ...
    <div className="p-6">
      ...
      {selectedDoc && editedDoc && (
        <ViewEditModal
          ...
          selectedAddItemId={selectedAddItemId}
          ...
          onEditDocChange={setEditedDoc}
          onEditItem={handleEditItem}
          onRemoveItem={handleRemoveItem}
          onAddCatalogItem={handleAddCatalogItem}
          onAddCustomItem={handleAddCustomItem}
          onSelectedAddItemIdChange={setSelectedAddItemId}
          onCustomItemChange={setCustomItem}
          onIsAddingCustomChange={setIsAddingCustom}
      ...
      {selectedDoc && (
        <DeliveryModal
          ...
          onDeliveryQtyChange={handleDeliveryQtyChange}
