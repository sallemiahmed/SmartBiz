...
import { Search, Plus, Eye, Trash2, X, ClipboardList, Printer } from 'lucide-react';
import { Invoice } from '../types';
import { useApp } from '../context/AppContext';
import { printInvoice } from '../utils/printGenerator';

interface SalesIssuesProps {
  onAddNew: () => void;
}
...
const SalesIssues: React.FC<SalesIssuesProps> = ({ onAddNew }) => {
  ...
  const issues = invoices.filter(inv => inv.type === 'issue');

  const filteredDocs = issues.filter(doc => {
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
              {filteredDocs.map((doc) => (
                <tr key={doc.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors group">
                  <td className="px-6 py-4 font-medium text-amber-600 dark:text-amber-400">{doc.number}</td>
                  <td className="px-6 py-4 text-gray-900 dark:text-white">{doc.clientName}</td>
                  <td className="px-6 py-4 text-gray-500">{doc.date}</td>
                  <td className="px-6 py-4 text-gray-500">{doc.items.length}</td>
