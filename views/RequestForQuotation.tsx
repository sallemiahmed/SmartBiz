      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-lg p-6">
        <div className="flex justify-between items-center mb-6 border-b border-gray-200 dark:border-gray-700 pb-4">
          <div>
            ...
            <span className="text-sm text-indigo-600 dark:text-indigo-400 font-mono">{rfq.number}</span>
          </div>
        ...
        <div className="space-y-4">
          <div className="flex justify-between">
            ...
            <span className="font-medium text-gray-900 dark:text-white">{rfq.supplierName}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-500">{t('date')}</span>
            <span className="font-medium text-gray-900 dark:text-white">{rfq.date}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-500">{t('deadline')}</span>
            <span className="font-medium text-gray-900 dark:text-white">{rfq.deadline || '-'}</span>
          </div>
          <div className="flex justify-between">
...
// Quote Entry Modal Component
interface QuoteEntryModalProps {
  isOpen: boolean;
  rfq: Purchase;
  quotedPrices: Record<string, number>;
  onClose: () => void;
...
const QuoteEntryModal = React.memo<QuoteEntryModalProps>(({
  isOpen,
  rfq,
  quotedPrices,
  onClose,
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-lg p-6">
        <div className="flex justify-between items-center mb-6 border-b border-gray-200 dark:border-gray-700 pb-4">
          <div>
            ...
            <p className="text-xs text-gray-500">RFQ: {rfq.number}</p>
          </div>
          <button onClick={onClose}><X className="w-5 h-5 text-gray-500" /></button>
        </div>
...
QuoteEntryModal.displayName = 'QuoteEntryModal';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-sm p-6 text-center">
         ...
         <p className="text-gray-500 dark:text-gray-400 mb-6 text-sm">
           {t('delete_rfq_msg')} <span className="font-bold">{rfq.number}</span>?
         </p>
